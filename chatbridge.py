...
def delete_caches():
    for chan_id in all_ids:
        all_ids[chan_id].bridgecache = None
...
def disable_bridge(bridge_id):
    delete_caches()
    loop.create_task(update_discord_topics())
    if bridge_id in enabled_bridges:
        enabled_bridges.remove(bridge_id)
    else:
        raise ValueError("Bridge '{}' does not exist".format(bridge_id))
...
def bridge_all(discord_server, bridge_id, strings, enabled=True):
    ...
    for string_id in strings:
        ...
        if clean_id == None:
            raise ValueError("Discord channel or Minecraft server not found: {}".format(string_id))
        else:
...
message_match = re.compile('^\[\d\d:\d\d:\d\d\] \[Server thread/INFO\]: <(§\w)?((\w)+)(§\w)?> (.*)$')
joined_match = re.compile('^\[\d\d:\d\d:\d\d\] \[Server thread/INFO\]: (§\w)?((\w)+)(§\w)? joined the game')
left_match = re.compile('^\[\d\d:\d\d:\d\d\] \[Server thread/INFO\]: (§\w)?((\w)+)(§\w)? left the game')
count_match = re.compile('^\[\d\d:\d\d:\d\d\] \[Server thread/INFO\]: There are (\d*)/\d* players online:')
info_match = re.compile('^\[\d\d:\d\d:\d\d\] \[Server thread/INFO\]: (.*)')
list_1_13_match = re.compile('^\[\d\d:\d\d:\d\d\] \[Server thread/INFO\]: There are (\d*) of a max \d* players online: (.*)')
def extract_message(string):
    match = message_match.fullmatch(string)
    if match:
        user = match.group(2)
...
def extract_joined(string):
    match = joined_match.fullmatch(string)
    if match:
        return match.group(2)
    return None

def extract_left(string):
    match = left_match.fullmatch(string)
    if match:
        return match.group(2)
    return None

def extract_playercount(string):
    match = count_match.fullmatch(string)
    if match:
        count = match.group(1)
        return int(count)
    return None
...
def extract_info(string):
    match = info_match.fullmatch(string)
    if match:
        return match.group(1)
    return None

def extract_1_13_list(string):
    match = list_1_13_match.fullmatch(string)
    if match:
...
class Chat:
    ...
    def send_greeting(self):
        self.send_text("Hi, I'm a Minecraft <-> Discord chat bridge.\n" + \
                "* to learn about commands type `!chat help`")
    ...
    def list_bridges(self):
        ...
        for bridge_id, bridge in bridges.items():
            ...
            bridge_desc.append("* {}: ({}) - {}".format(bridge_id, " ,".join(channel_names), enabled_str))
...
class MinecraftChat(Chat):
    ...
    async def connect(self, server_name):
        ...
        while True:
            try:
                ...
                traceback.print_exc()
                print("Could not connect to Minecraft server '{0}', path '{1}'".format(server_name, path))
    ...
    def send_message(self, sender_name, sender_channel, message):
        ...
        command = '/tellraw @a ["",{"text":"<","color":"white"},{"text":' + sender_esc + \
                ',"color":"white"},{"text":"@","color":"gray"},{"text":' + channel_esc + \
                ',"color":"gray"},{"text":"> ","color":"white"},{"text":' + message_esc + \
                ',"color":"none"}]'
        self.send_raw_command(command)
    ...
    def send_joined_message(self, who, channel):
        ...
        channel_esc = escape_tellraw(channel_name)
        command = '/tellraw @a ["",{"text":' + who_esc + \
                ',"color":"yellow"},{"text":" joined ","color":"yellow"},{"text":' + channel_esc + \
                ',"color":"yellow"}]'
        self.send_raw_command(command)
    ...
    def send_left_message(self, who, channel):
        ...
        channel_esc = escape_tellraw(channel_name)
        command = '/tellraw @a ["",{"text":' + who_esc + \
                ',"color":"yellow"},{"text":" left ","color":"yellow"},{"text":' + channel_esc + \
                ',"color":"yellow"}]'
        self.send_raw_command(command)

    def set_private(self, is_private):
        self.send_text("Chat is now private. Prefix messages with !!" if is_private else "Chat is now public")
...
def get_playerlist_for_channel(chat):
    ...
    return "{} {} online (".format(total, "Player" if total == 1 else "Players") + ", ".join(counts) + ") \n" + "\n".join(lists)
    ...
    for dchatid, discord_chat in all_ids.items():
        if isinstance(discord_chat, DiscordChat):
            try:
                ...
                print("Setting topic of channel channel {} to '{}'".format(discord_chat.get_short_name(),topic))
    ...
    try:
        ...
        if words[0] == "!bridge":
            if not message.author.server_permissions.manage_server:
                await client.send_message("You don't have 'manage_server' permissions")
                return
            if len(words) < 2:
                raise ValueError("Usage: !bridge add/del/enable/disable [...]")
            if words[1] == "add":
                if len(words) < 5:
                    raise ValueError("Usage: !bridge add <brige_name> <chat1> <chat2> [...]")
